...
def get_movies(filters, page, movies_per_page):
    ...
    if project:
        cursor = db.movies.find(query, project).sort(
            sort).skip(page * movies_per_page)
    else:
        cursor = db.movies.find(query).sort(sort).skip(page * movies_per_page)

    movies = cursor.limit(movies_per_page)
    return (list(movies), movies.count())
